...
def get_bbox_from_tile_cube(
    ...
    out_ds = out_ds.isel(
        orbit=valid_images.any(["obs", "tile"]).values,
        obs=valid_images.any(["orbit", "tile"]).values,
    )
    return out_ds


def extend_true_to_chunk_edges(arr: np.ndarray[bool], chunk_size: int, axis: int = -1):
    """Extend True values in a boolean array to the edges of chunks of given size."""
